...
int vm_enough_memory(long pages)
{
	 ...
	 *23/11/98NJC：稍微不那么愚蠢的算法，它试图做“TheRightThing”。
	 *而不是使用一半（缓冲区+缓存），使用最小值。允许额外的2%的数字植物的安全边际。
	 */
	 ...
	 * too small.
	 *这种双重计数：nrpages在页面缓存和交换空间中。同时，这补偿了交换空间过度分配（即“nr_swap_pages”太小）。
	 */
	 ...
	 *下面的代码没有考虑inode和dentry slab缓存中的空闲空间、slab缓存碎片、inode和dentrie，它们在VM负载下将变得可释放，等等。
	 */
 ...
 *  sys_brk()在大多数情况下不需要全局内核锁，
 *  除非应用程序正在执行一些令人讨厌的操作，比如试图取消已经映射到常规文件的区域的brk。
 *  在这种情况下，解映射将需要调用需要全局锁的文件系统例程。
 */
...
struct vm_area_struct * find_vma(struct mm_struct * mm, unsigned long addr)
{
	...
	if (mm) {
		...
		vma = mm->mmap_cache;//mmap_cache中是上次执行该函数获得的结果
		//vma不在的起始和终止地址之间
